...
def get_leaps_calls(ticker_obj):
    ...
    leaps_expiries = [d for d in ticker_obj.options if datetime.strptime(d, '%Y-%m-%d') > leaps_cutoff]
...
def get_iv_threshold(ticker):
    df = pd.read_sql_query('SELECT iv FROM iv_rsi_history WHERE ticker = ?', conn, params=(ticker,))
    if df.empty:
        return None
    return df['iv'].quantile(IV_LOW_PERCENTILE)

def get_average_rsi(ticker):
    df = pd.read_sql_query('SELECT rsi FROM iv_rsi_history WHERE ticker = ?', conn, params=(ticker,))
    if df.empty:
        return None
    return df['rsi'].mean()
...
def process_ticker(ticker_symbol):
    ...
    # Determine IV low threshold from history
    iv_threshold = get_iv_threshold(ticker_symbol)
    if iv_threshold is None:
        print("Not enough IV history yet to determine threshold.\n")
        return

    # Combine IV and RSI into a score
    # Normalize RSI to 0-1 scale with oversold RSI close to 1 (good)
    rsi_score = max(0, (RSI_OVERSOLD_THRESHOLD - rsi) / RSI_OVERSOLD_THRESHOLD)
    # Normalize IV score: 1 if IV <= threshold, else decrease linearly
    iv_score = 1.0 if current_iv <= iv_threshold else max(0, 1 - (current_iv - iv_threshold) / iv_threshold)

    combined_score = (iv_score * (1 - RSI_WEIGHT)) + (rsi_score * RSI_WEIGHT)

    print(f"Current IV: {current_iv:.2%}, Threshold IV: {iv_threshold:.2%}, RSI: {rsi:.2f}\n")
    print(f"Scores => IV: {iv_score:.2f}, RSI: {rsi_score:.2f}, Combined: {combined_score:.2f}\n")

    # Alert if combined score high enough (e.g., > 0.7)
    if combined_score > 0.7:
        message = (
            f"{ticker_symbol} LEAPS Call IV Alert!\n"
            f"Date: {date.today().isoformat()}\n"
            f"Expiry: {expiry}\n"
            f"Strike: {strike}\n"
            f"IV: {current_iv:.2%} (Low Threshold: {iv_threshold:.2%})\n"
            f"RSI: {rsi:.2f}\n"
